...
var effects = {
    ...
    createFall1: function createFall1() {
        var _this = this;
    ...
    createFall2: function createFall2() {
        var _this2 = this;
    ...
    createFall3: function createFall3() {
        var _this3 = this;
    ...
    createFall4: function createFall4() {
        var _this4 = this;
    ...
    createFall5: function createFall5() {
        var _this5 = this;
...
var cubeFall = {
    ...
    createTween: function createTween() {
        ...
        this.cubeFallTween = new mojs.Tween({
            ...
            onStart: function onStart() {
                _this6.cubeEl.style.opacity = 1;
            },
            onUpdate: function onUpdate(progress) {
                var bounceProgress = _this6.bouncyEasing(progress),
                    scaleProgress = _this6.scaleEasing(progress),
                    squashProgress = _this6.squashEasing(progress);

                mojs.h.setPrefixedStyle(_this6.cubeEl, 'transform', 'translate3d(0, ' + (-_this6.translateY + _this6.translateY * bounceProgress) + 'px, 0)');
                mojs.h.setPrefixedStyle(_this6.cubeSquashEl, 'transform', 'scaleX(' + (1 + squashProgress) + ') scaleY(' + (1 - squashProgress) + ')');
                mojs.h.setPrefixedStyle(_this6.shadowEl, 'transform', 'scale(' + (1 + scaleProgress + squashProgress) + ') rotateX(' + -28 * squashProgress + 'deg) rotateY(' + 28 * squashProgress + 'deg) translateZ(' + -4 * squashProgress + 'px)');
                _this6.shadowEl.style.opacity = Math.max(bounceProgress / 2, .1);
            }
        });
    ...
    createTween: function createTween() {
        ...
        this.cubeAnticipationTween = new mojs.Tween({
            ...
            onUpdate: function onUpdate(p) {
                var anticipationProgress = _this7.anticipationEasing(p),
                    anticipationYProgress = 1 - _this7.anticipatingYEasing(p);
                mojs.h.style(_this7.cubeEl, 'transform', 'translate3d(0, ' + -_this7.translateY * (1 - anticipationYProgress) + 'px, 0)');

                var nAnticipationProgress = 1 + (1 - anticipationProgress);
                mojs.h.style(_this7.cubeSquashEl, 'transform', 'scaleX(' + anticipationProgress + ') scaleY(' + nAnticipationProgress + ')');
                ...
                if (anticipationProgress > 1) {
                    ...
                    var scale = nAnticipationProgress,
                        rotateX = .2 * mojs.easing.cubic.out(p);
                }
                mojs.h.style(_this7.shadowEl, 'transform', 'scale(' + scale + ')\n                                  translateX(' + -.5 * anticipationProgress + 'px)\n                                  translateY(' + -.5 * anticipationProgress + 'px)\n                                  rotateX(' + 30 * rotateX + 'deg)\n                                  rotateY(' + -30 * rotateX + 'deg)');

                _this7.shadowEl.style.opacity = anticipationYProgress / 2 - .3 * (1 - anticipationProgress);
            }
        });
...
var cube = {
    ...
    vars: function vars() {
        ...
        window.addEventListener('resize', function() {
            _this8.translateY = window.innerHeight;
        });
    },
    createTween: function createTween() {
...
var doorOpen = {
    ...
    createTween: function createTween() {
        ...
        var tween = new mojs.Tween({
            ...
            delay: this.delay * this.s,
            onStart: function onStart() {
                _this9.isSound && _this9.doorOpenSound.play();
            },
            onUpdate: function onUpdate(p) {
                // return;
                var bounce = mojs.easing.bounce.out(p);
                var pp = mojs.easing.cubic.out(mojs.easing.cubic.out(p));
                mojs.h.setPrefixedStyle(_this9.doorEl, 'transform', 'rotateY(' + -125 * bounce + 'deg) translateX(' + 8 * (1 - pp) + 'px) translateY(' + 8 * (1 - pp) + 'px) scaleX(' + (1 - .25 * bounce) + ')');
...
var moleOpen = {
    ...
    createTween: function createTween() {
        ...
        var moleOpenMotionPath = new motionPathStagger({
            ...
            onStaggerStart: function onStaggerStart() {
                _this10.isSound && _this10.whooshSound1.play();
            }
        });
        ...
        var coneTween = new mojs.Tween({
            ...
            onUpdate: function onUpdate(p) {
                ...
                mojs.h.style(_this10.moleConeEl, 'transform', 'rotateX(' + 140 * coneP + 'deg) rotateY(' + -20 * coneP + 'deg) translateY(' + -6 * coneP + 'px)');
                mojs.h.style(_this10.moleConeShadowEl, {
                    opacity: 1 - coneP,
        ...
        var stretchTween = new mojs.Tween({
            ...
            onUpdate: function onUpdate(p) {
                ...
                mojs.h.setPrefixedStyle(_this10.moleInnerEl, 'transform', 'scaleX(' + stretchP + ') scaleY(' + inv + ') translateZ(0)');
            }
        });
...
var tongue = {
    ...
    vars: function vars() {
        this.delay = this.handRantingDuration + 275;
        this.duration = 1800;
    },
    createTween: function createTween() {
        var _this11 = this;
        ...
        var salivaLauncher = new mojs.Tween({
            delay: 435 * this.s,
            onStart: function onStart() {
                _this11.isSound && _this11['salivaSound' + Math.round(mojs.h.rand(1, 2))].play();
            }
        });
        ...
        var tween = new mojs.Tween({
            ...
            onStart: function onStart() {
                ...
                salivaLauncher.run();
                _this11.isSound && _this11.sighSound1.play();
            },
            onUpdate: function onUpdate(p) {
                ...
                mojs.h.style(_this11.moleEl, 'transform', 'skewX(' + (75 * skewP - 5 * noizeP) + 'deg) skewY(' + 15 * noizeP + 'deg) ' + _this11.zHack);
                mojs.h.style(_this11.moleHandEl, 'transform', 'rotate(' + (_this11.handAngle * handP + 30 * noizeP) + 'deg) ' + _this11.zHack);
                mojs.h.style(_this11.moleHandLeftEl, 'transform', 'rotate(' + (100 * skewP - 15 * noizeP) + 'deg) translateY(' + -100 * skewP + 'px) ' + _this11.zHack);
                mojs.h.style(_this11.moleMouthEl, 'transform', 'scale(' + (mouthP + 3 * noizeP) + ') translate(' + -30 * skewP + 'px, 0) ' + _this11.zHack);
                mojs.h.style(_this11.moleGlassesEl, 'transform', 'translateY(' + 20 * noizeP + 'px) ' + _this11.zHack);
                mojs.h.style(_this11.moleGlassesLeftEl, 'transform', 'translateY(' + 10 * noizeP + 'px) ' + _this11.zHack);

                mojs.h.style(_this11.moleEyeEl, 'transform', '');

                mojs.h.style(_this11.moleConeEl, 'transform', 'rotateZ(' + 20 * noizeP + 'deg) ' + _this11.zHack);

                mojs.h.style(_this11.moleTongueEl, 'transform', 'translate(' + -140 * skewP + 'px, 0) ' + _this11.zHack);
                _this11.moleTonguePath.setAttribute('d', 'M0 30 Q 20 ' + (10 + 550 * noizeP) + ', 30 25 T 50 30');

                // SQUINT
                var squintP = squintCurve(p);
                var n_squintP = mojs.easing.inverse(squintP);
                _this11.moleEyeLashEl.setAttribute('d', 'M0,0 Q 6.5 ' + 10 * squintP + ', 13 0');
                _this11.moleEyeLashEl.setAttribute('stroke-width', 2 + 1.5 * n_squintP);
                mojs.h.style(_this11.moleEyeEl, 'transform', 'rotate(' + 37 * n_squintP + 'deg) translate(' + 7 * n_squintP + 'px, ' + -4 * n_squintP + 'px) scaleX(' + (1 - .4 * n_squintP) + ') ' + _this11.zHack);
            }
        });
        // tween.start();
...
var moleRantingHand = {
    ...
    createTween: function createTween() {
        ...
        var tween = new mojs.Tween({
            ...
            onStart: function onStart() {
                _this12.isSound && _this12['rantSound' + Math.round(mojs.h.rand(1, 3))].play();
            },
            onUpdate: function onUpdate(p) {
                ...
                mojs.h.style(_this12.moleHandEl, 'transform', 'rotate(' + _this12.handAngle * handP + 'deg) scaleX(' + (1 - .4 * handStretchP) + ') scaleY(' + (1 + handStretchP) + ') ' + _this12.zHack);
                mojs.h.style(_this12.moleHandLeftEl, 'transform', 'translateY(' + -75 * handStretchP + 'px) ' + _this12.zHack);

                mojs.h.style(_this12.moleGlassesEl, 'transform', 'translateY(' + -45 * handStretchShiftP + 'px) ' + _this12.zHack);
                mojs.h.style(_this12.moleGlassesLeftEl, 'transform', 'translateY(' + -45 * handStretchShiftP + 'px) ' + _this12.zHack);

                var bodyScale = .25 * handStretchP;
                mojs.h.style(_this12.moleEl, 'transform', 'scaleY(' + (1 + bodyScale) + ') scaleX(' + (1 - bodyScale) + ') ' + _this12.zHack);
                mojs.h.style(_this12.moleMouthEl, 'transform', 'scale(' + Math.min(1 - 5 * handStretchP, 1.2) + ') translateX(' + Math.min(-50 * handStretchShiftP, 10) + 'px) ' + _this12.zHack);
                mojs.h.style(_this12.moleMouthInnerEl, 'transform', 'scale(' + (1 - 4 * handStretchP) + ') translateX(' + Math.min(-150 * handStretchP, 10) + 'px) translateY(' + -20 * handStretchShiftP + 'px) ' + _this12.zHack);
                mojs.h.style(_this12.moleMouthOuterEl, 'border-top-right-radius', 200 * handStretchShiftP + '%');
            }
        });
        this.moleRantingTween.add(tween);
    },
...
var moleRantingSearch = {
    ...
    createTween: function createTween() {
        ...
        var doorSound1Tween = new mojs.Tween({
            delay: 510 * this.s,
            onStart: function onStart() {
                _this13.isSound && _this13.doorOpenSoundSmall.play();
            }
        });
        ...
        var wehSound1Tween = new mojs.Tween({
            delay: 120 * this.s,
            onStart: function onStart() {
                _this13.isSound && _this13.wehSound1.play();
            }
        });
        ...
        var wehSound2Tween = new mojs.Tween({
            delay: 400 * this.s,
            onStart: function onStart() {
                _this13.isSound && _this13.wehSound2.play();
            }
        });
        ...
        var tween = new mojs.Tween({
            ...
            onUpdate: function onUpdate(p) {
                ...
                mojs.h.style(_this13.moleEl, 'transform', 'skewX(' + 9 * bodySkewP + 'deg) scale(' + (1 - .15 * riseP) + ', ' + (1 + .15 * riseP) + ') translate(0, ' + -17 * riseP + 'px) ' + _this13.zHack);
                mojs.h.style(_this13.moleHandEl, 'transform', 'rotate(' + (29.5 + 65 * bodySkewP) + 'deg) scaleY(' + (1 + .1 * bodySkewP) + ') translateY(' + -15 * riseP + 'px) ' + _this13.zHack);
                mojs.h.style(_this13.moleHandLeftEl, 'transform', 'rotate(' + 20 * bodySkewP + 'deg) translate(' + 30 * bodySkewP + 'px, ' + (-10 * bodySkewP - 10 * riseP) + 'px) ' + _this13.zHack);

                _this13.moleEyeLashEl.setAttribute('d', 'M0,0 Q ' + (6.5 - 13 * riseP) + ' ' + (10 - 30 * Math.max(riseP, 0)) + ', 13 0');
                _this13.moleEyeLashEl.setAttribute('stroke-width', 2 + 2.5 * riseP);
                mojs.h.style(_this13.moleEyeEl, 'transform', 'rotate(' + 60 * riseP + 'deg) translate(' + 7 * riseP + 'px, ' + -4 * riseP + 'px) scaleX(' + (1 - .9 * riseP) + ') ' + _this13.zHack);

                mojs.h.style(_this13.moleMouthEl, 'transform', 'scale(' + (.3 + 2.4 * riseP) + ') ' + _this13.zHack);
            }
        });
        ...
        var tween2 = new mojs.Tween({
            ...
            onUpdate: function onUpdate(p) {
                var riseP = riseC(p);
                mojs.h.setPrefixedStyle(_this13.moleConeEl, 'transform', 'rotateX(' + 210 * riseP + 'deg) ' + _this13.zHack);
                mojs.h.setPrefixedStyle(_this13.moleGlassesEl, 'transform', 'translateY(' + -25 * riseP + 'px) ' + _this13.zHack);
                mojs.h.setPrefixedStyle(_this13.moleGlassesLeftEl, 'transform', 'translateY(' + -25 * riseP + 'px) ' + _this13.zHack);
            }
        });
        ...
        var tween3 = new mojs.Tween({
            ...
            onUpdate: function onUpdate(p) {
                ...
                mojs.h.setPrefixedStyle(_this13.doorEl, 'transform', 'rotateY(' + (-125 + 90 * e.inverse(doorAngleP)) + 'deg) scaleX(.75) ' + _this13.zHack);
                mojs.h.setPrefixedStyle(_this13.doorShadowEl, 'transform', 'scaleX(' + (1 - .45 * e.inverse(doorAngleP)) + ') ' + _this13.zHack);
            }
        });
...
var close = {
    ...
    createTween: function createTween() {
        ...
        var closeSoundLauncher = new mojs.Tween({
            ...
            delay: 150 * this.s,
            onStart: function onStart() {
                _this14.isSound && _this14.doorCloseSound.play();
                _this14.isSound && _this14.fallingSound.play();
            }
        });
        ...
        var moveDownTween = new mojs.Tween({
            duration: 600 * this.s,
            onStart: function onStart() {
                _this14.moleHandCircleEl.style.opacity = 1;
            },
            onComplete: function onComplete() {
                _this14.resetParts();
                _this14.moleHandCircleEl.style.opacity = 0;
                h.style(_this14.moleHandEl, 'transform', '');
                h.style(_this14.moleHatEl, 'transform', '');
                _this14.moleHatEl.style['z-index'] = 4;
                _this14.moleEyeLashEl.setAttribute('d', 'M0,0 Q 6.5 10, 13 0');
                _this14.moleEyeLashEl.setAttribute('stroke-width', 2);
                mojs.h.style(_this14.moleEyeEl, 'transform', '');
                h.style(_this14.moleGlassesEl, 'transform', '');
                h.style(_this14.moleGlassesLeftEl, 'transform', '');
            },
            onUpdate: function onUpdate(p) {
                ...
                h.style(_this14.moleEl, 'transform', 'translate(' + 25 + 'px, ' + (-240 + 360 * cubicInP) + 'px) ' + _this14.zHack);
                // (p === 1) && (this.moleEl.style['z-index'] = 0);

                h.style(_this14.moleInnerEl, 'transform', 'skewX(' + 9 * (1 - p) + 'deg) scale(' + (1 - .2 * cubicInP) + ', ' + (1 + .2 * cubicInP) + ') ' + _this14.zHack);

                h.style(_this14.moleHandEl, 'transform', 'rotate(' + (94.5 + 100.5 * quadInP) + 'deg) scaleY(' + (1.1 - .1 * quadInP) + ') translateY(' + 70 * quadInP + 'px) ' + _this14.zHack);
                h.style(_this14.moleHandLeftEl, 'transform', 'rotate(' + (20 - 20 * quadInP) + 'deg) translate(' + (30 - 35 * quadInP) + 'px, ' + (-10 - 70 * quadInP) + 'px) ' + _this14.zHack);

                h.style(_this14.doorEl, 'transform', 'rotateY(' + (-125 + 125 * cubicInP) + 'deg) scaleX(' + (.75 + .25 * expoInP) + ') translate(' + 8 * expoInP + 'px, ' + 8 * expoInP + 'px) ' + _this14.zHack);
                h.style(_this14.doorShadowEl, 'transform', 'scaleX(' + (1 - 1.65 * cubicInP) + ') ' + _this14.zHack);

                h.style(_this14.moleConeEl, 'transform', 'rotateX(' + 150 * quadInP + 'deg) rotateY(' + -14 * quadInP + 'deg) translateY(' + -5 * quadInP + 'px) ' + _this14.zHack);
                _this14.moleConeShadowEl.style.opacity = 1 - p;
                h.style(_this14.moleHatEl, 'transform', 'translateY(' + -35 * expoInP + 'px) ' + _this14.zHack);
                _this14.moleHatEl.style['z-index'] = Math.round(12 - 12 * p);

                h.style(_this14.moleGlassesEl, 'transform', 'translateY(' + -85 * expoInP + 'px) ' + _this14.zHack);
                h.style(_this14.moleGlassesLeftEl, 'transform', 'translateY(' + -85 * expoInP + 'px) ' + _this14.zHack);

                _this14.moleEyeLashEl.setAttribute('d', 'M0,0 Q 6.5 ' + 10 * p + ', 13 0');
                _this14.moleEyeLashEl.setAttribute('stroke-width', 2 + 1.5 * p);
                mojs.h.style(_this14.moleEyeEl, 'transform', 'translate(' + 7 * p + 'px, ' + -5 * p + 'px) scaleX(' + (1 - .4 * p) + ') ' + _this14.zHack);
                mojs.h.style(_this14.moleMouthInnerEl, 'transform', 'scale(' + (.3 - .3 * p) + ') ' + _this14.zHack);
            }
        });
        ...
        for (var _iterator = burstStagger.childModules, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
            var _ref;
        ...
        var doorWaveTween = new mojs.Tween({
            duration: 400 * this.s,
            onUpdate: function onUpdate(p) {
                mojs.h.style(_this14.doorWaveEl, 'transform', 'scale(' + (1.1 + 2 * e.cubic.out(p)) + ') ' + _this14.zHack);
                _this14.doorWaveEl.style.opacity = 1 - e.expo.out(p);
            }
        });
        ...
        var noiseTween = new mojs.Tween({
            duration: 200 * this.s,
            onStart: function onStart() {
                _this14.isSound && _this14.boomSound.play();
            },
            onUpdate: function onUpdate(p) {
                mojs.h.style(_this14.sceneEl, 'transform', 'translate(' + noiseCurve(p) * 40 + 'px, ' + -noiseCurve(p) * 20 + 'px) ' + _this14.zHack);
            }
        });
